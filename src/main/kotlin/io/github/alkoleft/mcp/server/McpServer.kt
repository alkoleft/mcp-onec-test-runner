...
class McpServer(
    ...
    @Tool(
        name = "run_all_tests",
        description = "Запускает все тесты YaXUnit. Автоматически выполняет инкрементальную сборку при наличии изменений. Возвращает подробный отчет.",
    )
    fun runAllTests(
        @ToolParam(
    ...
    @Tool(
        ...
        description = "Запускает тесты YaXUnit указанного модуля. Автоматически выполняет инкрементальную сборку при наличии изменений.",
    )
    fun runModuleTests(
    ...
    @Tool(
        name = "dump_config",
        description = """Выгружает конфигурацию из ИБ в файлы исходников (ИБ → файлы).
            |Используйте для получения актуальных исходников после изменений в Конфигураторе.
            |Для загрузки изменений в базу используйте build_project.
            |Режимы: FULL — полная, INCREMENTAL — только изменения (по умолчанию), PARTIAL — по списку объектов.
            |Для нескольких расширений вызывайте по одному extension за вызов.""",
    )
    fun dumpConfig(
        @ToolParam(
